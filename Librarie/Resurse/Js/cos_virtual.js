        ...
        if (prod_sel){ //p.then(f1).then(f2).then(f3)
            ...
            .then(function(objson) {
                ...
                for (let prod of objson){
                    ...
                    imag.src=prod.imagine;
                    divImagine.appendChild(imag);
                    divCos.appendChild(divImagine);
                    let divInfo=document.createElement("div");
                    divInfo.innerHTML=`<p><b>${prod.nume}</b></p><p>Pret: ${prod.pret}</p><p>Culoare: ${prod.culoare}</p><p>Cantitate:  ${k}</p>`;
                    ...
                    //console.log("SELECTIE nOUA",vect)
                    StorageIdsProduse(vect);
                    location.reload(); //da refresh la pagina
                  };
